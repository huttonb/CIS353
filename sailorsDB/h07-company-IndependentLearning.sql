...
*/
SELECT * 
FROM (SELECT E.Ssn, E.Lname, E.Salary FROM employee E ORDER BY E.Salary DESC)
WHERE ROWNUM < 5;
/*(120) TOP-N query.
Find the ssn, lname, and salary of the four lowest paid employees
*/
SELECT * 
FROM (SELECT E.Ssn, E.Lname, E.Salary FROM employee E ORDER BY E.Salary)
WHERE ROWNUM < 5;
/*(125) TOP-N query.
Find the lowest two salaries in the company.(Notice that in our database, the two lowest salaries are 25K and 30K.)
*/
SELECT * 
FROM (SELECT DISTINCT E.Salary FROM employee E ORDER BY E.Salary)
WHERE ROWNUM < 3;
/*(130) TOP-N query.
...
FROM (SELECT DISTINCT E2.Salary FROM employee E2 ORDER BY E2.Salary)
WHERE ROWNUM < 3);
/*(135) RANK query
Find the rank of the salary 30000 among all salaries.
 (HINT: The ranks in our database are 1 for 25000, 4 for 30000, 5 for 38000, and so on.)
*/
SELECT RANK(30000) WITHIN GROUP (ORDER BY E.Salary) "Rank"
FROM EMPLOYEE E;

/*(140) RANK query ... compare with the previous query.
Find the rank of the salary 31000 among all salaries.
*/
SELECT RANK(31000) WITHIN GROUP (ORDER BY E.Salary) "Rank"
FROM EMPLOYEE E;
/*(145) DENSE RANK query
Find the dense rank of the salary 30000 among all salaries.
 Hint: The dense ranks in our database are 1 for 25000, 2 for 30000, 3 for 38000, and so on.
*/
SELECT DENSE_RANK(30000) WITHIN GROUP (ORDER BY E.Salary) "Dense_Rank"
FROM EMPLOYEE E;
/*(150) DENSE RANK query ... compare with the previous query.
Find the dense rank of the salary 31000 among all salaries. 
Hint: The dense ranks in our database are 1 for 25000, 2 for 30000, 3 for 38000, and so on.
*/
SELECT DENSE_RANK(31000) WITHIN GROUP (ORDER BY E.Salary) "Dense_Rank"
FROM EMPLOYEE E;
/*(155)HIERARCHICAL query (uses START WITH and CONNECT BY PRIOR)
Find pairs of SSN's such that the first SSN in the pair is that of an employee while
 the second SSN in the pair is that of his/her supervisor. Start with SSN 453453453.
